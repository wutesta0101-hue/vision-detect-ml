...
使用 Ultralytics YOLOv8s，在 2,114 張的 PPE 資料集上 fine-tune，test 集 mAP@50 達 0.866。  
訓練框架為 Ultralytics；**資料集統計、評估、失敗分析與跨實驗比較工具為本專案自行實作**。

**但這份報告的重點不是那個數字。** 三個發現比 mAP 更有價值：

| 發現 | 影響 |
|---|---|
| 誤報是漏檢的兩倍，但**成因因影像而異** | 不能用單一原因解釋，需逐張查證 |
| 已查證的 `Mask` 誤報來自**標註漏標**；`Safety_boots` 的漏檢是**模型本身** | 兩者改善方向完全相反 |
...
Early stopping 在 epoch 73 觸發。查 `results.csv`，**最佳輪次為 epoch 53**（val mAP@50 = 0.8947），`best.pt` 即取自該輪。
...
![PR 曲線](figures/ppe-2114-v1-pr.png)

*由 Ultralytics 產生，配色與其他圖表不一致——重畫需存取未文件化的內部 API，不划算（見 `evaluate.md`）*

曲線說明的是 precision 與 recall 的取捨：閾值調高則 precision 升、recall 降。  
`model-service` 目前固定在 conf = 0.25，對應的實際計數見下方部署閾值表。

若應用場景對漏檢的容忍度低（例如工地安全巡檢，漏掉一個未戴安全帽者的代價高於一次誤報），  
可依此曲線下調閾值——**但本專案未做此調整**，0.25 是沿用預設值，非依曲線選定。
...
失敗分析（`error_analysis.py`，conf = 0.25，IoU = 0.45，test 集）：
...
| Person | 9 | 23 | 2.6× |
| Gloves | 20 | 36 | 1.8× |
| Safety_boots | 29 | 32 | 1.1× |
| **合計** | **92** | **192** | **2.1×** |

> **與第三節部署表的 85 / 180 不同是預期的。** 兩者的配對規則不同：
>
> | | 第三節（Ultralytics 混淆矩陣） | 本節（`error_analysis.py`） |
> |---|---|---|
> | IoU 閾值 | 0.45 | 0.45（已對齊） |
> | 類別 | 先不分類別按 IoU 配對，再判類別對錯 | 只配對同類別 |
> | 去重 | 全域按 IoU 降序後雙向去重 | 逐個預測貪婪配對 |
>
> 閾值對齊後仍有 7 / 12 的差距，來自配對規則。**兩者都正確，  
> 但跨實驗比較時要固定用同一個來源**——第六節用的是部署表。
...
| 類別 | 已查證影像中的成因 | 改善方向 |
|---|---|---|
| Mask 誤報 | **標註漏標**（案例 B：6/6 的 IoU = 0.00） | 修資料 |
| Safety_boots 漏檢 | **模型漏檢**（案例 A：密集、小、遮擋） | 改訓練 |
| Vest 誤報 | 部分漏標 + 與 Person 邊界重疊 | 待進一步查證 |

**「誤報是漏檢的兩倍」不能用單一原因解釋。** 逐張查證前，任何解釋都只是猜測。

> **樣本基礎**：上述結論來自 2 張影像的逐框查證。  
> 它證明了「不同影像的錯誤成因確實不同」，但**不足以推斷各成因在 213 張 test 集中的比例**。  
> 要量化需抽樣 20–30 張人工核對，見第八節。
...
## 五、發現二：Mask 的低 mAP 至少有一部分是資料問題

`Mask` 的 test mAP@50 為 0.676，六類最低，且較 val（0.797）下降 0.12。

由案例 B 可知，**該張影像的 6 個 `Mask` 誤報全部來自標註漏標**（IoU = 0.00）——  
模型偵測到的口罩位置正確，但標註缺失。

`Mask` 全域誤報共 45 個，**其中漏標所佔比例未經統計**。  
因此只能說 `Mask` 的實際表現**可能優於 0.676**，而 precision（0.650，六類最低）  
**可能被低估**——低估的程度無法量化。

同時它仍有 17 個漏檢，那部分是模型本身的限制。

> 若僅閱讀 mAP 數字，會做出「Mask 最需要改善模型」的判斷。  
> 但已查證的證據顯示，至少有一部分問題出在**標註品質**。  
> 兩者的成本與作法完全不同，因此值得先量化比例再決定投入方向。
...
| 總時長 | 22 分 | 44.5 分 | +102% |
| GPU 推論 | ~5 ms | ~6 ms | 單次測量，變異大 |

> 訓練日誌中「最後一個 epoch 的 val mAP@50」為 v1 0.879、v2 0.886——方向與上表相反。  
> 那是**最後一輪**的驗證值，而 `best.pt` 取的是最佳輪次（v1 為 epoch 53）；  
> 結論依據為 test 集的獨立評估。
...
已查證的 `Mask` 誤報來自標註缺失——**兩者都不是提高像素能解決的問題。**
...
> 修補範圍三行。詳見 [`reports/smoke-report.md`](reports/smoke-report.md)。

**推論效能註記**：本報告的推論時間為 GPU 推論，且為單次測量。  
`model-service` 使用 CPU 推論，延遲量級不同，不可直接比較。
...
2. 誤報與漏檢的成因因影像而異：已查證的 `Mask` 誤報來自標註漏標，  
...
**未依 PR 曲線選定信心閾值。** 0.25 沿用 `model-service` 的預設值。
...
| **參數的理由要查證，不能用類比** | `error_analysis.py` 的 IoU 閾值原設 0.5，理由寫「與 mAP@50 一致」——那個類比不成立。查 Ultralytics 原始碼後改為 0.45 |
| **不同工具的計數不可混用** | 部署表 85、失敗分析 92，配對規則不同，兩者都對 |
...
rows = [{k.strip(): v for k, v in r.items()}
        for r in csv.DictReader(open('runs/detect/ppe-2114-v1/results.csv'))]
b = max(rows, key=lambda r: float(r['metrics/mAP50(B)']))
print('最佳 epoch:', b['epoch'], 'mAP50:', b['metrics/mAP50(B)'])
"
```
...
資料集來源與授權見 [`docs/SOURCES.md`](docs/SOURCES.md)。本資料集採 CC BY 4.0，引用需署名 SDP。
